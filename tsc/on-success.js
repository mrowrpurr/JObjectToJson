...
console.log(`Installing ${pluginName} module`);
if (!outDir) {
    writeFileSyncRecursive(path.join(seRoot, 'Data\\Platform\\Modules', "JObjectToJson.ts"), fs.readFileSync("JObjectToJson.ts"));
}
else {
    fs.copySync(`./JObjectToJson.ts`, path.join(seRoot, 'Data\\Platform\\Modules', "JObjectToJson.ts"));
}
